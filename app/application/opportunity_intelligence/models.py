...
class OpportunityIntelligenceInput:
    ...
    def __post_init__(self) -> None:
        if self.factors is not None and not isinstance(
            self.factors,
            OpportunityFactors,
        ):
        ...
        if self.factors is None and not self.missing_factors:
            raise ValueError(
                "factors가 없으면 missing_factors를 최소 하나 지정해야 합니다."
            )

        if self.factors is not None and self.missing_factors:
            raise ValueError(
                "완전한 factors와 missing_factors를 동시에 제공할 수 없습니다."
            )
...
class OpportunityIntelligenceResult:
    ...
    def __post_init__(self) -> None:
        ...
        if self.status is OpportunityIntelligenceStatus.EVALUATED:
            if self.score is None or self.evaluation is None:
                raise ValueError("evaluated 결과에는 score와 evaluation이 필요합니다.")
        ...
        if self.score is not None or self.evaluation is not None:
            raise ValueError(
                "evaluated가 아닌 결과에는 score와 evaluation을 넣을 수 없습니다."
            )

        if self.status is OpportunityIntelligenceStatus.UNAVAILABLE:
            if not self.missing_factors:
                raise ValueError(
                    "unavailable 결과에는 missing_factors가 필요합니다."
                )
            if self.error_message is not None:
                raise ValueError(
                    "unavailable 결과에는 error_message가 없어야 합니다."
                )
            return
